...
def generate_gitlab_ci_yaml(env, print_summary, output_file, prune_dag=False,
    ...
    for phase in phases:
        ...
        for stage_jobs in stages:
            ...
            for spec_label in stage_jobs:
                ...
                if broken_specs_url:
                    dag_hash = release_spec.dag_hash()
                    s3_key = '{0}/{1}'.format(broken_specs_url, dag_hash)
                    if web_util.url_exists(s3_key):
                        known_broken_specs_encountered.append('{0} ({1})'.format(
                            release_spec, dag_hash))
    ...
    if known_broken_specs_encountered:
        ...
        tty.debug('No known broken specs encountered')
